...
- Let the active report style control title and boundary decisions:
  ...
  - exploit-first may split operator-significant exploit paths into standalone findings when that materially improves reader understanding
- If exploit-first splits multiple presented findings out of one shared root cause, preserve that linkage through `Fingerprint`, `Related Findings`, and attack-chain references.
